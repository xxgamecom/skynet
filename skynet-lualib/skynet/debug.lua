...
local function init(skynet, export)
    ...
    local function init_dbgcmd()
        ...
        function dbgcmd.GC()
            ...
            skynet.log(string.format("GC %.2f Kb -> %.2f Kb, cost %.2f sec", before, after, (after_time - before_time) / 100))
            gcing = false
        end
        ...
        function dbgcmd.KILLTASK(threadname)
            ...
            if co then
                skynet.log(string.format("Kill %s", co))
                skynet.ret()
            else
                skynet.log(string.format("Kill %s : Not found", threadname))
    ...
    skynet.register_protocol {
        name = "debug",
        id = assert(skynet.MSG_PTYPE_DEBUG),
