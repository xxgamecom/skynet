...
-- protocols
local register_proto_table = {}

--
-- @param class protocol info
function skynet.register_protocol(class)
    ...
    -- check
    assert(register_proto_table[name] == nil and register_proto_table[id] == nil)
    assert(type(name) == "string" and type(id) == "number" and id >= 0 and id <= 255)

    --
    register_proto_table[name] = class
    register_proto_table[id] = class
end
...
local function co_create(f)
    -- get last coroutine in pool
    local co = table_remove(coroutine_pool)

    -- pool is empty, need create
    if co == nil then
...
function skynet.send(addr, typename, ...)
    local p = register_proto_table[typename]
...
skynet.redirect = function(dest, source, typename, ...)
    return c.redirect(dest, source, register_proto_table[typename].id, ...)
end
...
local function raw_dispatch_message(prototype, msg, sz, session, source)
    ...
    if prototype == 1 then
        ...
        local p = register_proto_table[prototype]
        if p == nil then
        ...
        if f then
            ...
            if session ~= 0 then
                ...
                unknown_request(session, source, msg, sz, register_proto_table[prototype].name)
            end
        end
    end
end
...
function skynet.traceproto(prototype, flag)
    local p = assert(register_proto_table[prototype])
    p.trace = flag
end
...
-- lua service entry
-- @param start_func entry function
