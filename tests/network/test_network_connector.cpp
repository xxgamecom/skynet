...
class connect_handler : public skynet::network::tcp_connector_handler
{
public:
...
class stream_handler : public skynet::network::tcp_session_handler
{
public:
...
int32_t main(int32_t argc, char* argv[])
{
    ...
    // ios pool
    skynet::network::io_service_pool ios_pool(1);
    ios_pool.run();

    // socket connector
    std::cout << "create connector" << std::endl;
    std::shared_ptr<connect_handler> connect_handler_ptr = std::make_shared<connect_handler>();
    std::shared_ptr<skynet::network::tcp_connector> connector_ptr = std::make_shared<skynet::network::tcp_connector>(ios_pool.select_one());
    ...
    std::shared_ptr<skynet::network::tcp_session> session_ptr = std::make_shared<skynet::network::tcp_session>(8192, 4096, 4);
