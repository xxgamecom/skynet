...
local function resolve(content)
    if #content < DNS_HEADER_LEN then
        -- drop
        skynet.log_error("Recv an invalid package when dns query")
        return
    end
    ...
    if question.name ~= resp.name then
        skynet.log_warn("Recv an invalid name when dns query")
    end
...
local function connect_server()
    ...
    dns_server.fd = fd
    skynet.log_info(string.format("Udp server open %s:%s (%d)", dns_server.address, dns_server.port, fd))
end
    ...
    local function check_alive()
        if skynet.now() > dns_server.retire + DNS_SERVER_RETIRE then
            ...
            if fd then
                dns_server.fd = nil
                socket.close(fd)
                skynet.log_warn(string.format("Udp server close %s:%s (%d)", dns_server.address, dns_server.port, fd))
            end
        else
