...
public:
    // 接收连接成功
    virtual void handle_tcp_server_accept(std::shared_ptr<skynet::net::tcp_session> session_ptr) override
    {
    ...
    // tcp会话读完成
    virtual void handle_tcp_server_session_read(std::shared_ptr<skynet::net::tcp_session> session_ptr, char* data_ptr, size_t data_len) override
    {
    ...
    // tcp会话写完成
    virtual void handle_tcp_server_session_write(std::shared_ptr<skynet::net::tcp_session> session_ptr, char* data_ptr, size_t data_len) override
    {
    }

    // tcp会话闲置
    virtual void handle_tcp_server_session_idle(std::shared_ptr<skynet::net::tcp_session> session_ptr, skynet::net::session_idle_type type) override
    {
        //std::cout << "session idle" << std::endl;
    }

    // tcp会话关闭
    virtual void handle_tcp_server_sessoin_close(std::shared_ptr<skynet::net::tcp_session> session_ptr) override
    {
...
{
private:
    std::shared_ptr<skynet::net::tcp_server> server_ptr_;
    int32_t block_size_ = 0;
...
//        server_ptr_ = skynet::net::create_tcp_server(svc_handle, socket_id, acceptor_ios_ptr, acceptor_ios_pool_ptr, net_manager_ptr, acceptor_config_ptr, session_config_ptr);
//
...
//        server_ptr_->set_event_handler(server_handler_ptr);
        ...
        //server_ptr_->session_config().write_buf_size(32*1024);
