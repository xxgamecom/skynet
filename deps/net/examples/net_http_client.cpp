...
public:
    // 主动连接成功
    void handle_tcp_client_connect_success(std::shared_ptr<skynet::net::tcp_session> session_ptr) override
    {
    ...
    // 主动连接失败
    void handle_tcp_client_connect_failed(std::shared_ptr<skynet::net::tcp_session> session_ptr, int32_t err_code, std::string err_msg) override
    {
        std::cout << fmt::format("connec failed({}:{})", err_code, err_msg) << std::endl;
    }

    // 超时处理
    void handle_tcp_client_connect_timeout(std::shared_ptr<skynet::net::tcp_session> session_ptr) override
    {
        std::cout << "connect timeout" << std::endl;
    }

    // tcp会话读完成
    void handle_tcp_client_read(std::shared_ptr<skynet::net::tcp_session> session_ptr, char* data_ptr, size_t data_len) override
    {
        std::cout << std::endl;
    ...
    // tcp会话写完成
    void handle_tcp_client_write(std::shared_ptr<skynet::net::tcp_session> session_ptr, char* data_ptr, size_t data_len) override
    {
        std::cout << "write completed bytes: " << data_len  << std::endl;
    }

    // tcp会话关闭
    void handle_tcp_client_close(std::shared_ptr<skynet::net::tcp_session> session_ptr) override
    {
...
int32_t main(int32_t argc, char* argv[])
{
    ...
    auto ios_ptr = skynet::net::create_io_service();

    // create client
    std::cout << "create client" << std::endl;
    auto service_handler_ptr = std::make_shared<tcp_client_handler>();
    auto client_ptr = skynet::net::create_tcp_client(0, ios_ptr);
