...
// tcp会话读完成
void tcp_client_impl::handle_session_read(std::shared_ptr<tcp_session> session_ptr, char* data_ptr, size_t data_len)
{
    // 读取会话回调
    if (event_handler_ptr_ != nullptr)
...
// tcp会话写完成
void tcp_client_impl::handle_session_write(std::shared_ptr<tcp_session> session_ptr, char* data_ptr, size_t data_len)
{
    // 写入会话回调
    if (event_handler_ptr_ != nullptr)
...
// tcp会话闲置
void tcp_client_impl::handle_session_idle(std::shared_ptr<tcp_session> session_ptr, idle_type type)
{
    // 客户端不需要闲置处理
}

// tcp会话关闭
void tcp_client_impl::handle_sessoin_close(std::shared_ptr<tcp_session> session_ptr)
{
    // 会话关闭回调
    if (event_handler_ptr_ != nullptr)
