...
size_t tcp_session_impl::write(const char* data_ptr, size_t data_len)
{
    // 确保只有在'打开'状态能写
...
// socket选项
bool tcp_session_impl::set_sock_option(sock_options opt, int32_t value)
{
    // socket没有打开
...
bool tcp_session_impl::get_sock_option(sock_options opt, int32_t& value)
{
    // socket没有打开
    if (socket_ptr_ == nullptr || socket_ptr_->is_open() == false) return false;
...
// 处理完成的写操作
void tcp_session_impl::handle_async_write(const asio::error_code& ec, size_t bytes_transferred)
{
    if (!ec)
    {
        // 更新写字节计数
