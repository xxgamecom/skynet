...
// 初始化
bool session_manager_impl::init(int32_t session_pool_size,
...
void session_manager_impl::fini()
{
...
std::shared_ptr <basic_session> session_manager_impl::create_session(session_type type)
{
    std::lock_guard <std::mutex> guard(sessions_mutex_);
...
//        session_ptr = udp_session_pool_ptr_->alloc();
    ...
    // alloc session id
    session_id_t id = create_session_id();
...
void session_manager_impl::release_session(std::shared_ptr <basic_session> session_ptr)
{
    std::lock_guard <std::mutex> guard(sessions_mutex_);
...
// 获取所有会话
size_t session_manager_impl::get_sessions(std::vector <std::weak_ptr<basic_session>>& sessions)
{
    std::lock_guard <std::mutex> guard(sessions_mutex_);
