...
{
protected:
    std::shared_ptr<io_service> ios_ptr_;                       // io service, only used for acceptor, must provider
...
public:
    ...
    bool open(std::string local_ip, uint16_t local_port, bool reuse_addr = true, int32_t backlog = DEFAULT_BACKLOG) override;
    void close() override;
