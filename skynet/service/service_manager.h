...
public:
    ...
    //                可以在 msg_ptype 里设上 alloc session 的 tag (MESSAGE_TAG_ALLOC_SESSION), send api 就会忽略掉传入的 session 参数，而会分配出一个当前服务从来没有使用过的 session 号，发送出去。
    //                同时约定，接收方在处理完这个消息后，把这个 session 原样发送回来。这样，编写服务的人只需要在 callback 函数里记录下所有待返回的 session 表，就可以在收到每个消息后，正确的调用对应的处理函数。
    int send(service_context* svc_ctx, uint32_t src_svc_handle, uint32_t dst_svc_handle, int msg_ptype, int session_id, void* msg, size_t msg_sz);
     ...
     * @param msg_sz
     */
    int send_by_name(service_context* svc_ctx, uint32_t src_svc_handle, const char* dst_name_or_addr, int msg_ptype, int session, void* msg, size_t msg_sz);
