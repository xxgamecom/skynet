...
void server::fini()
{
    ...
    {
        if (socket_ref.status != socket_status::ALLOCED)
        {
    ...
    for (;;)
    {
        ...
        if (need_check_ctrl_cmd_)
        {
            ...
            if (pipe_.is_readable())
            {
                ...
                // 需要清理 closed 事件
                if (type == socket_event_type::SOCKET_CLOSE || type == socket_event_type::SOCKET_ERROR)
                {
        ...
        //
        case socket_status::LISTEN:
            {
                int ok = report_accept(socket_ptr, result);
                if (ok > 0)
                    return socket_event_type::SOCKET_ACCEPT;
                if (ok < 0)
                    return socket_event_type::SOCKET_ERROR;
                
                // ok == 0, retry
                break;
            }
        case socket_status::FREE:
        ...
        default:
            ...
            if (event_ref.is_read)
            {
                ...
                else
                {
                    ...
                    // 尝试再次读取
                    if (type == socket_event_type::SOCKET_UDP)
                    {
                        --event_next_index_;
                        return socket_event_type::SOCKET_UDP;
                    }
                }

                // Try to dispatch write message next step if write flag set.
                if (event_ref.is_write && 
                    type != socket_event_type::SOCKET_CLOSE && 
                    type != socket_event_type::SOCKET_ERROR)
                {
                    event_ref.is_read = false;
                    --event_next_index_;
                }
            ...
            {
                force_close(socket_ptr, sl, result);
                return socket_event_type::SOCKET_CLOSE;
            }
    ...
    if (socket_ref.can_direct_write(socket_id))
    {
        ...
        if (sl.try_lock())
        {
            ...
            if (socket_ref.can_direct_write(socket_id))
            {
                ...
                {
                    socket_addr sa;
                    int sa_sz = udp_address_to_sockaddr(socket_ref.protocol, socket_ref.p.udp_address, sa);
                    if (sa_sz == 0)
                    {
    ...
    if (socket_ref.can_direct_write(socket_id))
    {
        ...
        if (sl.try_lock())
        {
            ...
            if (socket_ref.can_direct_write(socket_id))
            {
                ...
                send_object_init(&so, buf);
                socket_addr sa;
                socklen_t sa_sz = udp_address_to_sockaddr(socket_ref.protocol, udp_address, sa);
                if (sa_sz == 0)
                {
                    so.free_func((void*)buf->buffer);
                    return -1;
...
int server::handle_ctrl_cmd_open_socket(request_open* cmd, socket_message* result)
{
    ...
    do
    {
        ...
        if (status == 0)
        {
            new_socket_ptr->status = socket_status::CONNECTED;
            ...
            ::freeaddrinfo(ai_list);
            return socket_event_type::SOCKET_OPEN;
        }
        else
        {
            new_socket_ptr->status = socket_status::CONNECTING;
    ...
    {
        ::freeaddrinfo(ai_list);
        socket_slot_[calc_slot_index(socket_id)].status = socket_status::FREE;
        return socket_event_type::SOCKET_ERROR;
    }
...
int server::handle_ctrl_cmd_close_socket(request_close* cmd, socket_message* result)
{
    int socket_id = cmd->socket_id;
    auto& socket_ref = socket_slot_[calc_slot_index(socket_id)];
    if (socket_ref.status == socket_status::FREE || socket_ref.socket_id != socket_id)
    {
        result->socket_id = socket_id;
        result->svc_handle = cmd->svc_handle;
        result->ud = 0;
    ...
    //
    if (!socket_ref.has_nomore_sending_data())
    {
        int type = send_write_buffer(&socket_ref, sl, result);

        // -1 or socket_event_type::SOCKET_WARNING or socket_event_type::SOCKET_CLOSE, 
        //       socket_event_type::SOCKET_WARNING means has_nomore_sending_data
        if (type != -1 && type != socket_event_type::SOCKET_WARNING)
            return type;
    }

    //
    if (cmd->shutdown == 1 || socket_ref.has_nomore_sending_data())
    {
    ...
    socket_ref.status = socket_status::HALF_CLOSE;
...
int server::handle_ctrl_cmd_bind_socket(request_bind* cmd, socket_message* result)
{
    ...
    new_socket_ptr->status = socket_status::BIND;
...
int server::handle_ctrl_cmd_start_socket(request_start* cmd, socket_message* result)
{
    ...
    //
    if (socket_ref.status == socket_status::PACCEPT || socket_ref.status == socket_status::PLISTEN)
    {
        ...
        socket_ref.status = (socket_ref.status == socket_status::PACCEPT) ? socket_status::CONNECTED : socket_status::LISTEN;
        socket_ref.svc_handle = cmd->svc_handle;
        result->data = const_cast<char*>("start");
        return socket_event_type::SOCKET_OPEN;
    }
    //
    else if (socket_ref.status == socket_status::CONNECTED)
    {
    ...
    // if socket_ptr->status == socket_status::HALF_CLOSE , socket_event_type::SOCKET_CLOSE message will send later
    return -1;
}
...
int server::handle_ctrl_cmd_exit_socket(socket_message* result)
{
    ...
    return socket_event_type::SOCKET_EXIT;
}
...
int server::handle_ctrl_cmd_send_socket(request_send* cmd, socket_message* result, int priority, const uint8_t* udp_address)
{
    ...
    if (socket_ref.status == socket_status::FREE || socket_ref.socket_id != socket_id ||
        socket_ref.status == socket_status::HALF_CLOSE ||
        socket_ref.status == socket_status::PACCEPT)
    {
        so.free_func((void*)cmd->data_ptr);
        return -1;
    }
    //
    if (socket_ref.status == socket_status::PLISTEN || socket_ref.status == socket_status::LISTEN)
    {
    ...
    // socket已连接
    if (socket_ref.is_send_buffer_empty() && socket_ref.status == socket_status::CONNECTED)
    {
        // tcp
        if (socket_ref.protocol == protocol_type::TCP)
        {
    ...
    if (socket_ref.wb_size >= WARNING_SIZE && socket_ref.wb_size >= socket_ref.warn_size)
    {
        ...
        result->data = nullptr;
        return socket_event_type::SOCKET_WARNING;
    }
...
int server::handle_ctrl_cmd_listen_socket(request_listen* cmd, socket_message* result)
{
    ...
    new_socket_ptr->status = socket_status::PLISTEN;
...
void server::_clear_closed_event(int socket_id)
{
    for (int i = event_next_index_; i < event_wait_n_; i++)
    {
        ...
        if (socket_ptr->status == socket_status::FREE)
        {
...
void server::force_close(socket* socket_ptr, socket_lock& sl, socket_message* result)
{
    ...
    assert(socket_ptr->status != socket_status::ALLOCED);
    free_write_buffer_list(&socket_ptr->wb_list_high);
    free_write_buffer_list(&socket_ptr->wb_list_low);
    if (socket_ptr->status != socket_status::PACCEPT && socket_ptr->status != socket_status::PLISTEN)
    {
        event_poller_.del(socket_ptr->socket_fd);
    }
    
    sl.lock();
    if (socket_ptr->status != socket_status::BIND)
    {
        if (::close(socket_ptr->socket_fd) < 0)
            perror("close socket:");
    }
    socket_ptr->status = socket_status::FREE;
...
int server::_alloc_socket_id()
{
    for (int i = 0; i < MAX_SOCKET; i++)
    {
        ...
        if (socket_ref.status != socket_status::FREE)
            continue;

        // set socket status: alloced
        uint8_t expect_status = socket_status::FREE;
        if (socket_ref.status.compare_exchange_strong(expect_status, socket_status::ALLOCED))
        {
...
socket* server::new_socket(int socket_id, int sock_fd, int protocol, uint64_t svc_handle, bool add/* = true*/)
{
    ...
    {
        socket_ref.status = socket_status::FREE;
        return nullptr;
    }
...
int server::send_write_buffer_list_udp(socket* socket_ptr, write_buffer_list* wb_list, socket_message* result)
{
    while (wb_list->head != nullptr)
    {
        auto tmp = wb_list->head;
        socket_addr sa;
        socklen_t sa_sz = udp_address_to_sockaddr(socket_ptr->protocol, tmp->udp_address, sa);
        if (sa_sz == 0)
        {
...
int server::do_send_write_buffer(socket* socket_ptr, socket_lock& sl, socket_message* result)
{
    ...
    // step 1
    if (send_write_buffer_list(socket_ptr, &socket_ptr->wb_list_high, sl, result) == socket_event_type::SOCKET_CLOSE)
        return socket_event_type::SOCKET_CLOSE;
    ...
    if (socket_ptr->wb_list_high.head == nullptr)
    {
        ...
        {
            if (send_write_buffer_list(socket_ptr, &socket_ptr->wb_list_low, sl, result) == socket_event_type::SOCKET_CLOSE)
                return socket_event_type::SOCKET_CLOSE;
        ...
        if (socket_ptr->status == socket_status::HALF_CLOSE)
        {
            force_close(socket_ptr, sl, result);
            return socket_event_type::SOCKET_CLOSE;
        }
...
int server::report_accept(socket* socket_ptr, socket_message* result)
{
    ...
    new_socket_ptr->status = socket_status::PACCEPT;
    result->svc_handle = socket_ptr->svc_handle;
    result->socket_id = socket_ptr->socket_id;
    result->ud = socket_id;
    result->data = nullptr;
...
int server::report_connect(socket* socket_ptr, socket_message* result)
{
    ...
    // 
    socket_ptr->status = socket_status::CONNECTED;
    result->svc_handle = socket_ptr->svc_handle;
    result->socket_id = socket_ptr->socket_id;
    result->ud = 0;
    if (socket_ptr->has_nomore_sending_data())
    {
        event_poller_.write(socket_ptr->socket_fd, socket_ptr, false);
    }
    socket_addr sa;
...
int server::forward_message_tcp(socket* socket_ptr, socket_lock& sl, socket_message* result)
{
    ...
    return socket_event_type::SOCKET_DATA;
}
...
int server::forward_message_udp(socket* socket_ptr, socket_lock& sl, socket_message* result)
{
    ...
    if (sa_sz == sizeof(sa.v4))
    {
        ...
        sockaddr_to_udp_address(protocol_type::UDP, &sa, data_ptr + n);
    }
    // udp v6
    else
    {
        ...
        sockaddr_to_udp_address(protocol_type::UDPv6, &sa, data_ptr + n);
    }
...
bool server::_query_socket_info(socket& socket_ref, socket_info& si)
{
    ...
    case socket_status::CONNECTED:
        ...
        else
        {
            ...
            // 
            if (udp_address_to_sockaddr(socket_ref.protocol, socket_ref.p.udp_address, sa))
                to_endpoint(&sa, si.endpoint, sizeof(si.endpoint));
        }
        break;
    default:
