...
{
    int i;
    struct context* ctx = (context*)lua_touserdata(L, 2);
    struct table* tbl = ctx->tbl;
...
{
    struct context* ctx = (context*)lua_touserdata(L, 1);
...
static void convert_stringmap(struct context* ctx, struct table* tbl)
{
    ...
    lua_pushvalue(L, 1);
    struct state* s = (state*)lua_newuserdata(L, sizeof(*s));
    s->dirty = 0;
...
{
    struct table* tbl = (table*)lua_touserdata(L, index);
    if (tbl == NULL)
    {
...
{
    struct ctrl* c = (ctrl*)lua_touserdata(L, 1);
    struct table* tbl = c->root;
    struct state* s = (state*)lua_touserdata(tbl->L, 1);
    s->ref--;
    c->root = NULL;
    c->update = NULL;
...
static int lboxconf(lua_State* L)
{
    ...
    struct ctrl* c = (ctrl*)lua_newuserdata(L, sizeof(*c));
    c->root = tbl;
    c->update = NULL;
...
static int lupdate(lua_State* L)
{
    ...
    struct ctrl* c = (ctrl*)lua_touserdata(L, 1);
    struct table* n = (table*)lua_touserdata(L, 2);
    if (c->root == n)
    {
...
/**
 * skynet luaclib - skynet.sharedata.core
 */
...
static const luaL_Reg sharedata_funcs[] = {
    // used by host
    { "new",        lnewconf },
    { "delete",     ldeleteconf },
