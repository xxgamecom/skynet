    ...
    while (lua_next(L, -2) != 0)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            case LUA_TLIGHTUSERDATA:
                break;
            case LUA_TFUNCTION:
                if (lua_getupvalue(L, idx, 1) != nullptr)
                {
...
static int l_is_sharedtable(lua_State* L)
{
    int b = 0;
    if (lua_type(L, 1) == LUA_TTABLE)
    {
...
static int l_co_stackvalues(lua_State* L)
{
...
static int load_matrixfile(lua_State* L)
{
    ...
    if (source[0] == '@')
    {
        if (luaL_loadfilex_(L, source + 1, nullptr) != LUA_OK)
            lua_error(L);
    }
    else
    {
    ...
    if (top > 1)
    {
        ...
        for (i = 2; i <= top; i++)
        {
            ...
            case LUA_TLIGHTUSERDATA:
                ...
                break;
            case LUA_TFUNCTION:
                if (lua_iscfunction(L, i) && lua_getupvalue(L, i, 1) == nullptr)
                {
...
LUAMOD_API int luaopen_skynet_sharetable_core(lua_State* L)
{
    ...
    luaL_Reg l[] = {
        { "clone",          clone_table },
        { "stackvalues",    l_co_stackvalues },
        { "matrix",         matrix_from_file },
        { "is_sharedtable", l_is_sharedtable },
        { nullptr, nullptr },
    };
    luaL_newlib(L, l);
    return 1;
