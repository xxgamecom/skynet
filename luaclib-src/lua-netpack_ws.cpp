...
static struct queue* get_queue(lua_State* L)
{
    ...
    if (q == nullptr)
    {
        q = (queue*)lua_newuserdata(L, sizeof(struct queue));
        q->cap = QUEUE_SIZE;
        q->head = 0;
        q->tail = 0;
        int i;
...
{
    struct queue* nq = (queue*)lua_newuserdata(L, sizeof(struct queue) + q->cap * sizeof(struct netpack));
...
*/
static inline int read_size(uint8_t* buffer, int size, int* pack_head_length, int* mask, int* ismask, int* hasunmask_size)
{
...
static void push_more(lua_State* L, int socket_id, uint8_t* buffer, int size, int wsocket_handeshake)
{
    ...
    if (size < pack_size && !wsocket_handeshake)
    {
        ...
        memcpy(uc->pack.buf_ptr, buffer, uc->read);
        return;
    }
    ...
    if (size > 0)
    {
        push_more(L, socket_id, buffer, size, wsocket_handeshake);
    }
}
...
static int filter_data_(lua_State* L, int socket_id, uint8_t* buffer, int size, int wsocket_handeshake)
{
    struct queue* q = (queue*)lua_touserdata(L, 1);
    struct uncomplete* uc = find_uncomplete(q, socket_id);
    int pack_size = 0;
    ...
    if (uc != nullptr)
    {
        ...
        if (uc->read < 0)
        {
            ...
            if (wsocket_handeshake)
            {
                ...
                memcpy(uc->pack.buf_ptr, uc->header, uc->read);
            }
            else
            {
                uc->read = 0;
...
static inline int filter_data(lua_State* L, int socket_id, uint8_t* buffer, int size, int wsocket_handeshake)
{
    int ret = filter_data_(L, socket_id, buffer, size, wsocket_handeshake);
...
static int l_filter(lua_State* L)
{
    ...
    case SKYNET_SOCKET_EVENT_DATA:
        ...
        return filter_data(L, message->socket_id, (uint8_t*)buffer, message->ud, wsocket_handeshake);
...
static int l_pack(lua_State* L)
{
    ...
    uint8_t* buffer = (uint8_t*)skynet_malloc(len + pos);
...
static int l_getms(lua_State* L)
{
    struct timeval tv;
...
static const luaL_Reg netpack_funcs[] = {
    ...
    { "getms",    skynet::luaclib::l_getms },

    { nullptr,    nullptr },
};

LUAMOD_API int luaopen_skynet_netpack_ws(lua_State* L)
{
    luaL_checkversion(L);
