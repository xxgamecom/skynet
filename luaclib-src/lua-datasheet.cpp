...
{
    const struct table* t = (const table*)gettable(data, index);
    if (t == nullptr)
    {
    ...
    struct proxy* p = (proxy*)lua_newuserdata(L, sizeof(struct proxy));
...
static void update_cache(lua_State* L, const void* data, const void* newdata)
{
    ...
    while (lua_next(L, t) != 0)
    {
        ...
        {
            // pointer, table, proxy
            struct proxy* p = (proxy*)lua_touserdata(L, -1);
            if (p->data == data)
            {
                // update to newdata
                p->data = newdata;
                const struct table* newt = (table*)gettable(newdata, p->index);
                lua_pop(L, 1);
                // pointer, table
                clear_table(L);
                lua_pushvalue(L, lua_upvalueindex(1));
...
static int lupdate(lua_State* L)
{
    ...
    const char* newdata = (char*)lua_touserdata(L, 2);
...
static int lnew(lua_State* L)
{
    ...
    const char* data = (char*)lua_touserdata(L, 1);
    // hold ref to data
...
/**
 * skynet luaclib - skynet.datasheet.core
 */
...
static const luaL_Reg datasheet_funcs[] = {
    { "new",    lnew },
    { "update", lupdate },
...
LUAMOD_API int luaopen_skynet_datasheet_core(lua_State* L)
{
    ...
    luaL_newlibtable(L, datasheet_funcs);

    gen_metatable(L);
    luaL_setfuncs(L, datasheet_funcs, 1);
