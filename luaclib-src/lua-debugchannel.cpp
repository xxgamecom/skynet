...
{
    struct command* cmd = (command*) new char[sizeof(command) + sz];
    cmd->sz = sz;
    cmd->next = NULL;
    memcpy(cmd + 1, s, sz);
...
{
    struct channel_box* cb = (channel_box*)luaL_checkudata(L, 1, METANAME);
...
{
    struct channel_box* cb = (channel_box*)lua_touserdata(L, 1);
    if (cb)
    {
...
new_channel(lua_State* L, struct channel* c)
{
    ...
    struct channel_box* cb = (channel_box*)lua_newuserdata(L, sizeof(*cb));
    cb->c = c;
...
{
    struct channel* c = (channel*)lua_touserdata(L, 1);
    if (c == NULL)
...
/**
 * skynet luaclib - skynet.debugchannel
 */
