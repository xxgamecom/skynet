...
static int codec_sha256_encode(lua_State *L)
{
  ...
  SHA256((unsigned char *)cs, len, bs);
...
static int codec_sha1_encode(lua_State *L)
{
  size_t len;
  const char *cs = luaL_checklstring(L, 1, &len);
  unsigned char bs[SHA_DIGEST_LENGTH];
  char dst[SHA_DIGEST_LENGTH*2+1];
  
  SHA1((unsigned char *)cs, len, bs);
...
static int codec_rsa_private_decrypt(lua_State *L)
{
  ...
  int ret = RSA_private_decrypt(n, (unsigned char *)src, (unsigned char *)dst, rsa,RSA_SSLV23_PADDING);
  if(ret <= 0)
  {
    char cipher[1024] = {0};
    ERR_load_crypto_strings();
    ERR_error_string(ERR_get_error(), cipher);
    printf("%s\n\r",cipher);
    RSA_free(rsa);
    BIO_free_all(bio);
 ...
 * local src = 'something'
 * local pem = [[...]] --私钥PEM字符串
 * local bs = codec.rsa_sha256_private_sign(src, pem)
 * local dst = codec.base64_encode(bs) --BASE64签名
 */
 ...
 * local pem = [[...]] --公钥PEM字符串
 * local type = 1
 * local ok = codec.rsa_sha256_public_verify(src, bs, pem, type) --true/false
 */
 ...
 * local bs = codec.base64_decode(src)
 * local pem = [[...]] --私钥PEM字符串
 * local dst = codec.rsa_private_encrypt(bs, pem)
 */
static int codec_rsa_private_encrypt(lua_State *L)
{
  const char* src = luaL_checkstring(L, 1);
  const char* pem = luaL_checkstring(L, 2);
  RSA* rsa= NULL;
  ...
  int ret = RSA_private_encrypt(n, (unsigned char *)src, (unsigned char *)dst, rsa, RSA_PKCS1_PADDING);
 ...
 * local bs = codec.base64_decode(src)
 * local pem = [[...]] --公钥PEM字符串
 * local dst = codec.rsa_public_decrypt(bs, pem)
 */
static int codec_rsa_public_decrypt(lua_State *L)
{
  const char* src = luaL_checkstring(L, 1);
  const char* pem = luaL_checkstring(L, 2);
  ...
  int ret = RSA_public_decrypt(n, (unsigned char *)src, (unsigned char *)dst, rsa, RSA_PKCS1_PADDING); // RSA_NO_PADDING
  if(ret <= 0)
  {
    char cipher[128] = {0};
    printf("%s\n", ERR_error_string(ERR_get_error (),cipher));
  ...
  RSA_free(rsa);
  char relust[ret];
  strncpy(relust,dst,ret);
  lua_pushlstring(L, relust, ret);
  return 1;
}



static int codec_base64_decode_all(lua_State *L)
{
  size_t len;
  const char *cs = luaL_checklstring(L, 1, &len);
  BIO *b64 = BIO_new(BIO_f_base64());
 ...
 * local pem = [[...]] --公钥PEM字符串
 * local type = 1
 * local ok = codec.rsa_md5_sha1_verify(src, bs, pem, type) --true/false
 */


static int codec_rsa_md5_sha1_verify(lua_State *L)
{
  size_t srclen, signlen;
  const char* src = luaL_checklstring(L, 1, &srclen);
  const char* sign = luaL_checklstring(L, 2, &signlen);
  ...
  if (evpKey == NULL)
  {
    return luaL_error(L, "EVP_PKEY init error");
  }

  if (EVP_PKEY_set1_RSA(evpKey, rsa) != 1)
  {
      return luaL_error(L, "EVP_PKEY set error");
  }

  EVP_MD *mdtype = (EVP_MD *) EVP_md5();
  EVP_VerifyInit(md_ctx, mdtype);
  EVP_VerifyUpdate(md_ctx, src, srclen);
  int ret = EVP_VerifyFinal (md_ctx, (unsigned char *)sign, signlen, evpKey);
...
int luaopen_codec(lua_State *L)
{
  ...
  luaL_Reg l[] = {
    ...
    {"rsa_private_encrypt",codec_rsa_private_encrypt},
    {"rsa_public_decrypt",codec_rsa_public_decrypt},
    {"base64_decode_all",codec_base64_decode_all},
    {"rsa_md5_sha1_verify", codec_rsa_md5_sha1_verify},
