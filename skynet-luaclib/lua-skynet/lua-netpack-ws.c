...
static void push_more(lua_State* L, int fd, uint8_t* buffer, int size, int wsocket_handeshake)
{
...
static int filter_data_(lua_State* L, int fd, uint8_t* buffer, int size, int wsocket_handeshake)
{
    struct queue* q = lua_touserdata(L, 1);
    struct uncomplete* uc = find_uncomplete(q, fd);
    int pack_size = 0;
...
static inline int filter_data(lua_State* L, int fd, uint8_t* buffer, int size, int wsocket_handeshake)
{
