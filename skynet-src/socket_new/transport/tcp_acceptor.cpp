...
tcp_acceptor::tcp_acceptor(std::shared_ptr<io_service> ios_ptr,
...
// 打开acceptor
bool tcp_acceptor::open(const std::string local_ip,
...
// 投递一次异步accept
void tcp_acceptor::accept_once(std::shared_ptr<tcp_session> session_ptr)
{
    ...
    acceptor_ptr_->async_accept(*(session_ptr->get_socket()),
                                std::bind(&tcp_acceptor::handle_async_accept,
                                            shared_from_this(), session_ptr, std::placeholders::_1));
}

// socket选项
bool tcp_acceptor::set_sock_option(sock_options opt, int32_t value)
{
    // socket没有打开
...
bool tcp_acceptor::get_sock_option(sock_options opt, int32_t& value)
{
    // socket没有打开
    if (acceptor_ptr_ == nullptr || acceptor_ptr_->is_open() == false) return false;
...
// 处理接收
void tcp_acceptor::handle_async_accept(std::shared_ptr<tcp_session> session_ptr, const asio::error_code& ec)
{
    if (!ec)
    {
