...
tcp_connector::tcp_connector(std::shared_ptr<io_service> ios_ptr)
:
ios_ptr_(ios_ptr),
...
bool tcp_connector::connect(std::shared_ptr<tcp_session> session_ptr,
                            ...
                            const uint16_t local_port/* = 0*/)
{
    ...
    // 地址解析, 目前只支持第一个地址的连接, 以后可以增加第一个地址连接不成功时使用其他地址尝试连接
    asio::ip::tcp::resolver::query query(remote_addr,std::to_string(static_cast<uint32_t>(remote_port)));
    resolver_.async_resolve(query, std::bind(&tcp_connector::handle_async_resolve,
                                             shared_from_this(), session_ptr, std::placeholders::_1, std::placeholders::_2));

    return true;
}

void tcp_connector::handle_async_resolve(std::shared_ptr<tcp_session> session_ptr,
                                         ...
                                         const asio::error_code& ec)
{
    ...
    if (!ec)
    {
        ...
        // 开始异步连接
        session_ptr->get_socket()->async_connect(remote_ep, std::bind(&tcp_connector::handle_async_connect,
...
void tcp_connector::handle_async_connect(std::shared_ptr<tcp_session> session_ptr, const asio::error_code& ec)
{
    // 停止连接定时器
    stop_connect_timer();
...
void tcp_connector::handle_timeout(std::shared_ptr<tcp_session> session_ptr, const asio::error_code& ec)
{
    // 停止连接定时器
    stop_connect_timer();
