...
// 设置外部服务处理器
void tcp_server::set_event_handler(std::shared_ptr<tcp_server_handler> event_handler_ptr)
{
    event_handler_ptr_ = event_handler_ptr;
}

bool tcp_server::open(const std::string local_uri, bool is_reuse_addr/* = true*/)
{
...
// 打开服务
bool tcp_server::open(const std::string local_ip, const uint16_t local_port, bool is_reuse_addr/* = true*/)
{
    return open({ std::make_pair(local_ip, local_port) }, is_reuse_addr);
}

bool tcp_server::open(std::initializer_list<std::pair<std::string, uint16_t>> local_endpoints, bool is_reuse_addr/* = true*/)
{
    ...
    do
    {
        ...
        for (auto& itr : local_endpoints)
        {
            ...
            // 创建acceptor
            acceptor_ptr = std::make_shared<tcp_acceptor>(acceptor_ios_ptr_, shared_from_this());
...
// 接收连接成功
void tcp_server::handle_accept_success(std::shared_ptr<tcp_acceptor> acceptor_ptr,
...
// 接收连接失败
void tcp_server::handle_accept_failed(std::shared_ptr<tcp_acceptor> acceptor_ptr,
                                      std::shared_ptr<tcp_session> session_ptr,
...
// tcp会话读完成
void tcp_server::handle_session_read(std::shared_ptr<tcp_session> session_ptr, char* data_ptr, size_t data_len)
{
    // 回调给外部处理
    if (event_handler_ptr_ != nullptr)
...
// tcp会话写完成
void tcp_server::handle_session_write(std::shared_ptr<tcp_session> session_ptr, char* data_ptr, size_t data_len)
{
    // 回调给外部处理
    if (event_handler_ptr_ != nullptr)
...
// tcp会话闲置
void tcp_server::handle_session_idle(std::shared_ptr<tcp_session> session_ptr, idle_type type)
{
    // 回调给外部处理
    if (event_handler_ptr_ != nullptr)
...
// tcp会话关闭
void tcp_server::handle_sessoin_close(std::shared_ptr<tcp_session> session_ptr)
{
    // 回调给外部处理
    if (event_handler_ptr_ != nullptr)
